...
void Images_To_Binary(int Q_number, int k, const vector<bool> probe)           //Metatrepoume to dianusma ths apostashs se bianry dianusma diastashs k
{
    ...
    {        
        for(int i = 0; i < Dim; i++)                                           
        {    
           if (Random[Images[j][i]] == -2)                                     //An den mas exei ksana erthei o arithmos i tote kalooume thn rand 
            {
                int r = rand() % 2;
                ...
                Random[Images[j][i]] = r;                                      //Apothikeuoume to apotelesma ths rand ston pinaka
            }
            else if(Images[j][i] != 0)                                         //An mas exei erthei o arithmos tote den ksana kaloume thn rand apla briskoume thn timh pou exoume apothikeusei ston pinaka
            {
                BinaryImages[j][i%k] = Random[Images[j][i]];
            }
        }
    }
    for(int i = 0; i < Dim; i++)                                               //Kanoume thn idia diadikasia kai gia to query
    {    
        if (Random[Q_Image[Q_number][i]] == -2)
        {
            int r = rand() % 2;
            BinaryQuery[i%k] = r;
            Random[Q_Image[Q_number][i]] = r;
        }
        else if(Q_Image[Q_number][i] != 0)
        {
            BinaryQuery[i%k] = Random[Q_Image[Q_number][i]];
        }
    }
}
...
void Random_projection_to_HyperCube(int p, int k, int Q_number)                //Ekteloume ton algorithmo Hypercube
{
    ...
    {       
        for (size_t i = 0; i < searchSpace.size(); ++i)                        //Gia oles tis eikones pou uparxoun sth sugkekrimenh perioxh 
        {
            ...
            if (distance < bestDistance)                                       //Briskoume thn mikroterh apostash kai ton kontinotero geitona
            {
    ...
    bestDistance = Euclidian_dist_initial(nearestNeighbor, Q_number);
    Cube_Neighbor = nearestNeighbor;
...
void Random_projection_to_HyperCube_N(int Q_number, int p, int k, int N)       //Psaxnoume tous N kontinoterous geitones
{
    ...
    for (int probe = 0; probe < p; probe++)                                    //Psaxnoume oles tis korufes
    {
        for (size_t i = 0; i < searchSpace.size(); ++i)                        //Gia oles tis eikones pou uparxoun sth sugkekrimenh perioxh 
        {
            ...
            if(k_dist.size() < N)                                              //Kratame tis N prwtes apostaseis se enan pinaka kai tous arithmous twn eikonwn se enan allo         
            {
            ...
            else                                                               //An o pinakas autos exei gemisei 
            {
                ...
                if(k_dist.size() == N && distance < maxElement && find(N_Points.begin(), N_Points.end(), i) == N_Points.end())
                {    
    ...
    double bestDistance = Euclidian_dist_initial(N_Points[N-1], Q_number);
...
void Random_projection_to_HyperCube_RangeSearch(int Q_Number, int p, int k, int R)     //Ekteloume ton algorithmo tou uperkubou me Range Search
{
    ...
    for (int probe = 0; probe < p; probe++)                                            //Gia kathe korufh
    {
        for (size_t i = 0; i < searchSpace.size(); ++i)                                //Gia oles tis eikones pou uparxoun sth sugkekrimenh perioxh 
        {
            ...
            if (distance < R)                                                          //An h apostash einai mirkoterh apo to R kratame thn eikona
            {
                Flag = 0;
