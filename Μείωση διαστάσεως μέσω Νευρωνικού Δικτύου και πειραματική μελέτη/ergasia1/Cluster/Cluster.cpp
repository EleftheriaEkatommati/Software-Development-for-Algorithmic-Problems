...
long long int Euclidian_dist(int input_number1, int input_number2)                      //Ypologizoume thn eukleidia apostash metaksi 2 dianusmatwn diastashs Dim
{
    long long int dist = 0;
    for(int i = 0; i < Dim; i++)
    {
        dist += (Images[input_number1][i]-centroids[input_number2][i])*(Images[input_number1][i]-centroids[input_number2][i]);
    }
    return sqrt(dist);
}
...
void k_MeansPlusPlus_Init(int k)                                                        //Arxikopoihsh centroids
{
    ...
    for(int i = 0; i < Dim; i++)                                                        //Arxikopoioume to prwto kentro me mia tuxaia eikona
        centroids[0][i] = Images[rand() % NUMBER_IMAGES][i];
    for(int i = 1; i < k; i++)
    {
        ...
        for(size_t j = 0; j < NUMBER_IMAGES; j++)
        {
            ...
            {
                for(int l = 0; l < Dim; l++)
                    centroids[i][l] = Images[j][l];                                    //arxikopoioume to epomeno kentro me thn eikona sthn opoia mhdenithke h apostash 
                break;
            }
        } 
    }
...
void updateCentroids(int k)                                                             //Auth h sunarthsh xrhsimopoieitai gia ton algorithmo tou Classic clustering enw h prohgoumenh gia LSh                       
{
    ...
    for(size_t i = 0; i < NUMBER_IMAGES; i++)                                           //Gia kathe eikona
    {
        ...
        for(int j = 0; j < Dim; j++)
        {
            newCentroids[clusterIndex][j] = newCentroids[clusterIndex][j] + Images[i][j];    //Prosthetoume tis suntetagmenes olwn twn eikonwn pou einai sto idio cluster
        }
    }
...
double Euclidian_dist_between_images(int input_number1, int input_number2)              //Ypologizoume thn eukleidia apostash metaksi duo eikonwn         
{
    ...
    for(int i = 0; i < Dim; i++)
    {
        dist += (Images[input_number1][i]-Images[input_number2][i])*(Images[input_number1][i]-Images[input_number2][i]);
    }
    return sqrt(dist);
}
