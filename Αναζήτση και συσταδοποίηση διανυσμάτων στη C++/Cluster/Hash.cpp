...
long long int Euclidian_dist_LSH(int input_number, int centroid_number, vector<vector<double>> centroids)                       //Ypologizoume thn eukleidia apostash metaksi 2 dianusmatwn diastashs 784
{
    long long int dist = 0;
    for(int i = 0; i < Dim; i++)
    {
        dist += (Images[input_number].pixels[i]-centroids[centroid_number][i])*(Images[input_number].pixels[i]-centroids[centroid_number][i]);
    }
    return sqrt(dist);
}
...
void Images_To_Binary(int k_cube, const vector<bool> probe, vector<vector<double>> centroids, int K)
{
    ...
    {        
        for(int i = 0; i < 784; i++)
        {    
            if (Random[Images[j].pixels[i]] == -2)
            {
                int r = rand() % 2;
                BinaryImages[j][i%k_cube] = r;
                Random[Images[j].pixels[i]] = r;
            }
            else if(Images[j].pixels[i] != 0)
            {
                BinaryImages[j][i%k_cube] = Random[Images[j].pixels[i]];
            }
        }
    }
    for(int l = 0; l < K; l++)
