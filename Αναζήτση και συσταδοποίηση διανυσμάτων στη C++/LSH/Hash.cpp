...
{   
    ...
    for(int i = 0; i < L; i++)
    {
        ...
        for(int j = 0 ; j < k ; j++)
        {
            ...
            {
                x = Images[number_image-1].pixels[m];
...
long long int Euclidian_dist(int input_number, int query_number)                       //Ypologizoume thn eukleidia apostash metaksi 2 dianusmatwn diastashs 784
{
    ...
    for(int i = 0; i < Dim; i++)
    {
        dist += (Images[input_number-1].pixels[i]-Q_Image[query_number-1].pixels[i])*(Images[input_number-1].pixels[i]-Q_Image[query_number-1].pixels[i]);
    }
    return sqrt(dist);
}
...
void Euclidian_Nearest_N(int Q_number)                             
{
    ...
    for(long long unsigned int i = 1; i <= NUMBER_IMAGES; i++)
    {
        Dist = Euclidian_dist(Min_Number_Image, Q_number);
        if(Dist < Min_dist)
        {  
            Min_dist = Dist;
            Min_Number_Image = i;
    ...
    for (int i = 0; i < L; i++)                                                        //Psaxnoume ola ta hash tables gia to query
    {
        ...
        for(auto j = Euclidian_Hash_Tables[i][Pos_HT].begin(); j != Euclidian_Hash_Tables[i][Pos_HT].end(); j++)
        {
            ...
            {   
                ...
                if(k_dist.size() == N && Dist < maxElement && find(N_Points.begin(), N_Points.end(), *j) == N_Points.end())
                {    
                    k_dist[max_w] = Dist;
                    N_Points[max_w] = *j;
                }
...
void Euclidian_NN(int Q_number, int N)                                                 //exhaustive gia N-nearest neighbors
{
    ...
    for (long long unsigned int i = 1; i <= NUMBER_IMAGES; i++)
    {
        ...
        else
        {
            ...
            if(i == N && Dist < maxElement && find(NN.begin(), NN.end(), i) == NN.end())
            {
                k_dist[max_w] = Dist;
                NN[max_w] = i;
            }
